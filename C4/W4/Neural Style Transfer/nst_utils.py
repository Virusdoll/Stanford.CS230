...
def load_vgg_model(path):
    ...
    # def _conv2d_relu(x, layer, layer_name):
    #     W, b = _weights(layer, layer_name)
    #     layer = keras.layers.Conv2D(filters=W.shape[-1], kernel_size=W.shape[:-2],
    ...
    # def _avgpool(x):
    #     return keras.layers.AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding='SAME')(x)

    # Constructs the graph model.
    graph = {}
    # graph['input']   = tf.Variable(np.zeros((1, CONFIG.IMAGE_HEIGHT, CONFIG.IMAGE_WIDTH, CONFIG.COLOR_CHANNELS)), dtype = 'float32')
    graph['input'] = keras.Input(shape=(CONFIG.IMAGE_HEIGHT, CONFIG.IMAGE_WIDTH, CONFIG.COLOR_CHANNELS), dtype='float32')
    ...
    return keras.Model(graph['input'], graph['avgpool5'])
